<?php 
/**
* @version		$Id: grid.class.php 2440 2022-03-05 16:54:34Z IOS $
* @package		Elxis
* @subpackage	Grid
...
class elxisGrid {
	...
	private $percentage = 'auto';//false:use pixels, true: use percentage column widths, 'auto': auto (calculate on render)
	private $dir = 'ltr';
	private $i18n = array();
	...
	public function setOption($option, $value) {
		switch (strtolower($option)) {
			...
			break;
			case 'newp'://start page
				$value = (int)$value;
				$this->newp = ($value > 0) ? $value : 1;
			break;
			case 'width':
			case 'height':
				if (is_int($value)) {
			...
			case 'percentage': $this->$option = (bool)$value; break;
			default: break;
		}
	}
	...
	public function render() {
		...
		if ($this->percentage == true) {
			echo '<table id="'.$this->element.'" style="display: none; width:100%;"></table>'."\n";
		} else {
		...
		foreach ($this->columns as $name => $col) {
			...
			if ($this->percentage == true) {//percentage columns
				if ($last_col == $name) {
					echo "\t".'{ display:\''.addslashes($col['title']).'\', name:\''.$name.'\', width:\'auto\', sortable:'.$stxt.', align:\''.$col['align'].'\'}';
				} else {
					$pcw = intval((100 * $col['width']) / $total_width);
					echo "\t".'{ display:\''.addslashes($col['title']).'\', name:\''.$name.'\', width:\''.$pcw.'%\', sortable:'.$stxt.', align:\''.$col['align'].'\'}';
				}
			} else {
		...
		if ($this->newp > 1) { echo 'newp: '.$this->newp.",\n"; }
		...
		$stxt = ($this->percentage == true) ? 'true' : 'false';
		echo 'percentage: '.$stxt.",\n";

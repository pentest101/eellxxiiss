<?php 
/**
* @version		$Id: menu.xml.php 2284 2019-05-12 16:39:13Z IOS $
* @package		Elxis
* @subpackage	Menu/XML
...
class elxisXMLMenu {
	...
	private function makeParams($attrs, $parent, $ordering, $expand) {
		...
		$params['iconfont'] = (isset($attrs['iconfont'])) ? (string)$attrs['iconfont'] : '';
		return $params;
	}
...
/************ LINK GENERATOR FOR ADMIN AREA *************************/
	...
	public function getAllMenus($component, $section='frontend', $lang='') {
		...
		foreach ($xmenus as $xmenu) {
			...
			$menu->iconfont = $xmenu['iconfont'];
			$menu->cpiconfont = $xmenu['cpiconfont'];
	...
	private function parseAllMenus($section, $lang) {
		...
		foreach ($xml->$section->menu as $mchild) {
			...
			if ($attrs && isset($attrs['name'])) {
				...
				$iconfont = isset($attrs['iconfont']) ? (string)$attrs['iconfont'] : '';
				$cpiconfont = isset($attrs['cpiconfont']) ? (string)$attrs['cpiconfont'] : '';
			} else {
				break;
			}
			$menus[] = array(
	...
	private function parseEverything($xmenu, $lang) {
		...
		foreach ($xml_items as $xml_item) {
			...
			if (!$attrs) { continue; }
			if (!isset($attrs['link'])) { continue; }
			if (isset($attrs['if_file_exists'])) {
				if ($attrs['if_file_exists'] != '') {
					if (!file_exists(ELXIS_PATH.'/'.$attrs['if_file_exists'])) { continue; }
				}
			}

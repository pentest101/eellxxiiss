<?php 
/**
* @version		$Id: users.db.php 1767 2016-02-01 18:01:22Z sannosi $
* @package		Elxis
* @subpackage	Database
...
class usersDbTable extends elxisDbTable {
	...
	public function __construct() {
		...
		$this->columns = array(
			...
			'lastclicks' => array('type' => 'text', 'value' => null)
		);
	...
	public function fullCheck() {
		...
		if ($this->country == '') {
			...
			$this->country = filter_var($this->country, FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_LOW);
			$this->country = eUTF::trim(preg_replace($pat, '', $this->country));
		}
		if ($this->city == '') {
			$this->city = null;
		} else {
			$this->city = filter_var($this->city, FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_LOW);
			$this->city = eUTF::trim(preg_replace($pat, '', $this->city));
		}
		if ($this->address == '') {
			$this->address = null;
		} else {
			$this->address = filter_var($this->address, FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_LOW);
			$this->address = eUTF::trim(preg_replace($pat, '', $this->address));
		}
		if ($this->postalcode == '') {
			$this->postalcode = null;
		} else {
			$this->postalcode = filter_var($this->postalcode, FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_LOW);
			$this->postalcode = eUTF::trim(preg_replace($pat, '', $this->postalcode));
		}
		...
		if ($this->occupation == '') {
			...
			$this->occupation = filter_var($this->occupation, FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_LOW);
			$this->occupation = eUTF::trim(preg_replace($pat, '', $this->occupation));
		}
		...
		if ($uid < 1) {
			$rulesfile = ELXIS_PATH.'/includes/libraries/elxis/defender/email.rules.php';
			if (file_exists($rulesfile)) {
				...
				if (isset($rules) && is_array($rules) && (count($rules) > 0)) {
					...
					foreach ($rules as $rule) {
						if ($rule == $emaildomain) {
							$this->errorMsg = 'Use of temporary email providers like '.$rule.' is forbidden!';
							return false;
						}
					}
				}
	...
	public function addUserClick($returnclicks=false) {
		$clicks = array();
		if (trim($this->lastclicks) != '') { $clicks = json_decode($this->lastclicks, true); }
		if ($clicks) {
			if (count($clicks) > 9) {
				$olderts = 0;
				$olderidx = -1;
				...
				unset($clicks[$olderidx]);
			}
		}

		$url = eFactory::getURI()->getRealUriString();
		$clicks[] = array('ts' => time(), 'page' => $url);

		$this->lastclicks = json_encode($clicks);
		//$this->setValue('lastclicks', json_encode($clicks));
		if ($returnclicks) { return $this->lastclicks; }
	}

<?php 
/**
* @version		$Id: pathway.class.php 1528 2014-05-31 18:59:29Z sannosi $
* @package		Elxis
* @subpackage	Pathway
...
class elxisPathway {
	...
	/* SET YOU ARE HERE */
	/********************/
	public function setYouAreHere($you_are_here=false) {
		$this->you_are_here = (bool)$you_are_here;
	}
	...
	public function getHTMLNodes($auto_make_if_empty=true) {
		...
		foreach ($this->nodes as $idx => $node) {
			...
			if (($x == 0) && ($this->homeImage != '')) {
				...
				$txt = '<img src="'.$this->homeImage.'" alt="'.$node->title.'"'.$jstxt.' />';
			} else {
				$txt = $node->title;
			}
			if ($node->link == '') {

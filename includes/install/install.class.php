...
class elxisInstaller {
	...
	private function prepareEnv() {
		...
		if (!$found) {
			if (isset($_SERVER['HTTPS'])) {
				...
				if (strtolower($_SERVER['REQUEST_SCHEME']) == 'https') { $protocol = 'https'; }
			}
		}
	...
	public function getLang($str) {
		if (isset($this->langarr[$str])) { return $this->langarr[$str]; }
		return $str;
	}
	...
	public function process() {
		...
		if ($this->step == 3) {
			...
			if ($this->data['cfg']['cfg_repo_path'] == '') {
	...
	private function catchConfigPost() {
		...
		if (($cfg['cfg_encrypt_method'] != 'xor') && ($cfg['cfg_encrypt_method'] != 'mcrypt') && ($cfg['cfg_encrypt_method'] != 'openssl')) { $cfg['cfg_encrypt_method'] = ''; }
		...
		if ($cfg['cfg_ftp_root'] == '') { $cfg['cfg_ftp_root'] = '/'; }
		...
		if ($cfg['cfg_mail_method'] == '') { $cfg['cfg_mail_method'] = 'mail'; }
		if (!in_array($cfg['cfg_mail_method'], array('mail', 'smtp', 'sendmail'))) { $cfg['cfg_mail_method'] = 'mail'; }
		...
		if ($cfg['cfg_smtp_port'] < 1) { $cfg['cfg_smtp_port'] = 25; }
		...
		if ($cfg['cfg_smtp_secure'] != '') {
			if (!in_array($cfg['cfg_smtp_secure'], array('ssl', 'tls', 'starttls'))) { $cfg['cfg_smtp_secure'] = ''; }
		}
		...
		if ($cfg['cfg_mail_auth_method'] != '') {
			if (!in_array($cfg['cfg_mail_auth_method'], array('CRAM-MD5', 'LOGIN', 'NTLM', 'PLAIN', 'XOAUTH2'))) { $cfg['cfg_mail_auth_method'] = ''; }
		}
		...
		if ($cfg['cfg_repo_path'] != '') {//this is empty, we will create on save
		...
		if (isset($_POST['cfg_sitelangs'])) {
			...
			if ($cfg['cfg_sitelangs'] == '') {
				$cfg['cfg_multilinguism'] = 1;
			} else if ($cfg['cfg_sitelangs'] == 'none') {
				$cfg['cfg_sitelangs'] = $cfg['cfg_lang'];
				$cfg['cfg_multilinguism'] = 0;
			} else {
				$parts = explode(',', $cfg['cfg_sitelangs']);
				if (!in_array($cfg['cfg_lang'], $parts)) { $parts[] = $cfg['cfg_lang']; }
				$cfg['cfg_sitelangs'] = implode(',', $parts);
				if (count($parts) == 1) { $cfg['cfg_multilinguism'] = 0; }
			}
		}
	...
	private function catchUserPost() {
		...
		if (($pstr != $usr['u_pword']) || (trim($_POST['u_pword']) !=  $usr['u_pword'])) {
	...
	public function getiLangs() {
		return $this->ilangs ? array_keys($this->ilangs) : array();
	}
	...
	private function renameEstia() {
		$admin_folder = 'estia';
		if (!file_exists(ELXIS_PATH.'/estia/')) { return 'estia'; } 
		if (!is_dir(ELXIS_PATH.'/estia/')) { return 'estia'; }

		$names = array('acropolis', 'arcadia', 'utopia', 'valhalla', 'stargate', 'nirvana', 'enigma', 'mystic');
		$f = rand(1, 20);
		shuffle($names);
		$newname = $names[1].''.$f;
		if (file_exists(ELXIS_PATH.'/'.$newname.'/')) { return 'estia'; } //dont rename if there is already such a folder
	...
	private function renameRepository() {
		if ($this->data['cfg']['cfg_repo_path'] != '') { return; }
		if (!file_exists(ELXIS_PATH.'/repository/')) { return; } 
	...
	private function makeRewriteFile($sef, $admin_folder) {
		...
		if (!file_exists(ELXIS_PATH.'/includes/install/data/'.$file_initial)) { return false; }
		...
		while (!feof($handle)) {
			$line = fgets($handle);
			$line = preg_replace( "/\r|\n/", '', $line);
			if ($admin_folder != 'estia') { $line = str_replace('estia', $admin_folder, $line); }
			if ($this->relpath != '') { $line = str_replace('RewriteBase /', 'RewriteBase /'.$this->relpath.'/', $line); }
			$data .= $line."\r\n";
		}
		fclose($handle);

		if (file_exists(ELXIS_PATH.'/'.$file_final)) {
			$ok = @unlink(ELXIS_PATH.'/'.$file_final);
			if (!$ok) { return false; }
		}

		if ($handle = @fopen(ELXIS_PATH.'/'.$file_final, 'w')) {
			@fwrite($handle, $data);
			fclose($handle);
			return true;
		}

<?php 
/**
* @version		$Id: main.php 1481 2014-03-22 19:09:54Z sannosi $
* @package		Elxis
...
class mainSearchController {
	...
	public function run($engine) {
		...
		foreach ($engines as $engine => $data) {
			if (($engine != 'content') && ($engine != 'images') && ($engine != 'youtube')) {
				$eSearch->loadEngineLang($engine);
				$uppereng = strtoupper($engine);
				if ($eLang->exist($uppereng)) {
					$engines[$engine]['title'] = $eLang->get($uppereng);
					$eSearch->setEngineTitle($engine, $engines[$engine]['title']);
				}
			}
		}

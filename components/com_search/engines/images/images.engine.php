...
class imagesEngine implements searchEngine {
	...
	public function showResults() {
		...
		echo '<div class="elx5_ieng_container">'."\n";
		foreach ($this->results as $row) {
			echo '<div class="elx5_ieng_box">'."\n";
			echo '<div class="elx5_ieng_imgbox">'."\n";
			echo '<a href="'.$elxis->secureBase().'/'.$row->image.'" data-mediabox="resultimage" data-title="'.$row->title.'">';
			echo '<img src="'.$elxis->secureBase().'/'.$row->image.'" alt="'.$row->title.'" />';
			echo "</a>\n";
			echo "</div>\n";
			echo '<div class="elx5_ieng_notes">'."\n";

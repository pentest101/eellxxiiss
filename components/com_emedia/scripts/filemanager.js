 ...
 *	@copyright	Authors
 *  @Improved by Ioannis Sannos for Elxis CMS (on-the-fly config, resize image, and more)
 */
...
var loadConfigFile = function (type) {
	...
	if(type == 'user') {
		if (typeof elxisfmcfg != "undefined") {//elxis custom config already loaded in JSON format
			var userconfig = 'elxis';
			return elxisfmcfg;
		} else if($.urlParam('config') != 0) {
			var url = '/components/com_emedia/scripts/' + $.urlParam('config');
			var userconfig = $.urlParam('config');
		} else {
			var url = '/components/com_emedia/scripts/filemanager.config.js';
			var userconfig = 'filemanager.config.js';
		}
	} else {
		var url = '/components/com_emedia/scripts/filemanager.config.js.default';
		var userconfig = '';
	}
...
var fileConnector = config.options.fileConnector || config.options.elxisurl+'/components/com_emedia/connectors/' + config.options.lang + '/filemanager.' + config.options.lang;

// Read capabilities from config files if exists
// else apply default settings
var capabilities = config.options.capabilities || new Array('select', 'download', 'rename', 'resize', 'move', 'delete', 'replace');

// Get localized messages from file 
// through culture var or from URL
if($.urlParam('langCode') != 0) {
	if(file_exists ('/components/com_emedia/scripts/languages/'  + $.urlParam('langCode') + '.js')) {
		config.options.culture = $.urlParam('langCode');
	} else {
		var urlLang = $.urlParam('langCode').substring(0, 2);
		if(file_exists ('/components/com_emedia/scripts/languages/'  + urlLang + '.js')) config.options.culture = urlLang;
	}
}

var lg = [];
$.ajax({
  url: config.options.elxisurl+'/components/com_emedia/scripts/languages/'  + config.options.culture + '.js',
  async: false,
  dataType: 'json',
...
// 'cap' is one of 'select', 'rename', 'resize', delete', 'download', move
function has_capability(data, cap) {
	if (data['File_Type'] == 'dir' && cap == 'resize') return false; //isannos
	if (data['File_Type'] == 'dir' && cap == 'move') return false; //isannos: removed move capability from folders
	if (data['File_Type'] == 'dir' && cap == 'replace') return false;
	if (data['File_Type'] == 'dir' && cap == 'download') {
		if(config.security.allowFolderDownload == true) return true;
		else return false;
	}

	if (cap == 'resize') {
		if (data['File_Type'].toLowerCase() == 'jpg') { return true; }
		if (data['File_Type'].toLowerCase() == 'jpeg') { return true; }
		if (data['File_Type'].toLowerCase() == 'jpe') { return true; }
		if (data['File_Type'].toLowerCase() == 'png') { return true; }
		if (data['File_Type'].toLowerCase() == 'gif') { return true; }
		return false;
	}
...
var getVideoPlayer = function(data) {
	var code  = '<video width=' + config.videos.videosPlayerWidth + ' height=' + config.videos.videosPlayerHeight + ' src="'+config.options.elxisurl+'/'+config.options.elxisrelpath+''+ data['Path'] + '" controls="controls">';
...
var getAudioPlayer = function(data) {
	var code  = '<audio src="'+config.options.elxisurl+'/'+config.options.elxisrelpath+''+data['Path'] + '" controls="controls">';
		code += '<img src="' + data['Preview'] + '" />';
		code += '</audio>';
	$("#fileinfo img").remove();
	$('#fileinfo #preview #main-title').before(code);
...
var getPdfReader = function(data) {
	var code  = '<iframe id="fm-pdf-viewer" src = "'+config.options.elxisurl+'/components/com_emedia/scripts/ViewerJS/index.html#'+config.options.elxisurl+'/'+config.options.elxisrelpath+''+ data['Path'] + '" width="' + config.pdfs.pdfsReaderWidth + '" height="' + config.pdfs.pdfsReaderHeight + '" allowfullscreen webkitallowfullscreen></iframe>';
	$("#fileinfo img").remove();
	$('#fileinfo #preview #main-title').before(code);
};
...
var selectItem = function(data) {
	if(config.options.baseUrl !== false ) {
		//var url = smartPath(config.options.baseUrl, data['Path'].replace(fileRoot,""));//i. sannos
		var url = config.options.baseUrl+''+data['Path']; //i.sannos
	} else {
		var url = data['Path'];
	}
...
// Resizes the current item (image only) and returns the new name.
// Called by clicking the "Resize" button in detail views
// or choosing the "Resize" contextual menu option in 
// list views. Written by Ioannis Sannos (elxis.org)
var resizeItem = function(data) {
	var isResized = false;
	var curWidth = 0;
	if (data['Properties']['Width'] && (data['Properties']['Width'] != '')) { curWidth = data['Properties']['Width']; }
	var curHeight = 0;
	if (data['Properties']['Height'] && (data['Properties']['Height'] != '')) { curHeight = data['Properties']['Height']; }

	var msg = lg.width+': <input id="rwidth" name="rwidth" type="text" value="'+curWidth+'" /><br />';
	msg += lg.height+' : <input id="rheight" name="rheight" type="text" value="'+curHeight+'" /><br />';
	msg += 'Tip: Set Width or Height to 0 to resize keeping aspect ratio.'
	...
	var getNewSize = function(v, m){
		...
		var rheight = m.children('#rheight').val();
		rheight = parseInt(rheight, 10);
		if (rheight < 0) { rheight = 0; }
		
		if ((rwidth > 0) || (rheight > 0)) {
			var connectString = fileConnector + '?mode=resize&path='+encodeURIComponent(data['Path'])+'&rwidth='+rwidth+'&rheight='+rheight+'&config=' + userconfig;
			...
			$.ajax({
				...
				error: function (jqXHR, textStatus, errorThrown) {
					$.prompt('Error resizing image');
				},
				success: function(result){
					if (result['Code'] == 0) {
						isResized = true;
						data['Properties']['Width'] = result['New_Width'];
						data['Properties']['Height'] = result['New_Height'];
						$('#fileinfo').find('button#rename, button#resize, button#delete, button#download').unbind();
						bindToolbar(data);
						if(config.options.showConfirmation) $.prompt('Successfull resize');
						$('#filetree').find('a[data-path="' + result['Dirname'] +'/"]').click().click();
					} else {
						$.prompt(result['Error']);
					}
				}
...
//	$('body').prepend($form);
	...
	$('#toolbar').ajaxForm({
		...
		beforeSubmit: function (arr, form, options) {
			...
			if (getExtension(newFile) != data["File_Type"]) {
...
var getFileInfo = function(file) {
	...
	if($.inArray('resize', capabilities)  != -1 && config.options.browseOnly != true) template += '<button id="resize" name="resize" type="button" value="Resize">' + lg.resize + '</button>';
	...
	template += '</form>';
	template += '<img /><div id="main-title"><h1></h1><div id="tools"></div></div><dl></dl></div>';
...
i.sannos
			if(data['Protected']==0) {
				...
				loadJS(config.options.elxisurl+'/components/com_emedia/scripts/zeroclipboard/copy.js?d' + d.getMilliseconds());
...
var getFolderInfo = function(path) {
	...
	var loading = '<img id="activity" src="'+config.options.elxisurl+'/components/com_emedia/themes/' + config.options.theme + '/images/wait30trans.gif" width="30" height="30" />';
		...
		if(data){
			...
			if($('#fileinfo').data('view') == 'grid'){
				...
				for(key in data){
					...
					var props = data[key]['Properties'];
					var cap_classes = "";
					var title = config.options.showTitleAttr ? ' title="'+data[key]['Path']+'"' : '';
		...
		if(data) {
			...
			for(key in data) {
				...
				if (data[key]['File_Type'] == 'dir') {
					...
					result += "<li class=\"file ext_" + data[key]['File_Type'].toLowerCase() + extraclass + "\"><a href=\"#\" class=\"" + cap_classes + "\" data-path=\"" + data[key]['Path'] + "\">" + data[key]['Filename'] + "</a></li>";
					}
				}
			}
			result += "</ul>";
...
$(function(){
	...
	// Loading theme
	loadCSS(config.options.elxisurl+'/components/com_emedia/themes/' + config.options.theme + '/styles/filemanager.css');
	$.ajax({
		url: config.options.elxisurl+'/components/com_emedia/themes/' + config.options.theme + '/styles/ie.css',
		async: false,
		success: function(data)
		{
			$('head').append(data);
	...
	//loadJS(config.options.elxisurl+'/components/com_emedia/scripts/zeroclipboard/dist/ZeroClipboard.js');//i.sannos
	
	// Loading CodeMirror if enabled for online edition
	if(config.edit.enabled) {
		loadCSS(config.options.elxisurl+'/components/com_emedia/scripts/CodeMirror/lib/codemirror.css');
		loadCSS(config.options.elxisurl+'/components/com_emedia/scripts/CodeMirror/theme/' + config.edit.theme + '.css');
		loadJS(config.options.elxisurl+'/components/com_emedia/scripts/CodeMirror/lib/codemirror.js');
		loadJS(config.options.elxisurl+'/components/com_emedia/scripts/CodeMirror/addon/selection/active-line.js');
		loadCSS(config.options.elxisurl+'/components/com_emedia/scripts/CodeMirror/addon/display/fullscreen.css');
		loadJS(config.options.elxisurl+'/components/com_emedia/scripts/CodeMirror/addon/display/fullscreen.js');
		loadJS(config.options.elxisurl+'/components/com_emedia/scripts/CodeMirror/dynamic-mode.js');
	}
	...
	/** load searchbox */
	if(config.options.searchBox === true)  {
		loadJS(config.options.elxisurl+'/components/com_emedia/scripts/filemanager.liveSearch.min.js');
	} else {
		$('#search').remove();
	}
	...
	if(config.upload.multiple) {
		...
		loadCSS(config.options.elxisurl+'/components/com_emedia/scripts/dropzone/downloads/css/dropzone.css');
		loadJS(config.options.elxisurl+'/components/com_emedia/scripts/dropzone/downloads/dropzone.min.js');
	...
	if(config.customScrollbar.enabled) {
		loadCSS(config.options.elxisurl+'/components/com_emedia/scripts/custom-scrollbar-plugin/jquery.mCustomScrollbar.min.css');
		loadJS(config.options.elxisurl+'/components/com_emedia/scripts/custom-scrollbar-plugin/jquery.mCustomScrollbar.concat.min.js');

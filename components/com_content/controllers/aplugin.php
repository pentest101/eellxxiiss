...
class apluginContentController extends contentController {
	...
	private function handlerPlugin() {
		$id = (isset($_POST['id'])) ? (int)$_POST['id'] : 0;
		$fn = (isset($_GET['fn'])) ? trim($_GET['fn']) : '';
		if ($id < 1) { echo '<div class="elx5_error">No plugin selected!</div>'; return; }
		$row = $this->model->getPlugin($id);
		if (!$row) {
			echo '<div class="elx5_error">The requested plugin was not found!</div>';
			return;
		}
		if (!file_exists(ELXIS_PATH.'/components/com_content/plugins/'.$row->plugin.'/'.$row->plugin.'.plugin.php')) {
			echo '<div class="elx5_error">A required file for plugin <strong>'.$row->plugin.'</strong> was not found!</div>';
			return;
		}
		...
		$class = $row->plugin.'Plugin';
		if (!class_exists($class, false)) {
			echo '<div class="elx5_error">Class <strong>'.$class.'</strong> was not found. <strong>'.$row->plugin.'</strong> is not a valid Elxis plugin!</div>';
			return;
		}

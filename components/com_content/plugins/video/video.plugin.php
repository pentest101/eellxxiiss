<?php 
/**
* @version		$Id: video.plugin.php 2138 2019-03-06 19:58:50Z IOS $
* @package		Elxis
...
class videoPlugin implements contentPlugin {
	...
	public function handler($pluginid, $fn) {
		...
		$sess_token = trim($eSession->get('token_plugvidform'));
	...
	private function listVideos() {
		...
		if ($files) {
			...
			foreach ($files as $file) {
				...
				if (($ext != '') && (in_array($ext, $exts))) {
					$fs = filesize(ELXIS_PATH.'/'.$relpath.$file) / 1024;
					if ($fs > 600) {
						$fs = $fs / 1024;
						$size = round($fs, 2).' mb';
					} else {
					...
					$mfiles[] = array('name' => $file, 'size' => $size);
				}
			}
		}
		unset($files);
		...
		echo $htmlHelper->tableHead($eLang->get('SIZE'), 'elx5_nosorting elx5_tabhide');
		echo $htmlHelper->tableHead($eLang->get('ADD'), 'elx5_nosorting elx5_center');
		echo "</tr>\n";
		echo "</thead>\n";
		echo "<tbody>\n";
		...
		if ($mfiles) {
			foreach ($mfiles as $mfile) {
				echo '<tr>'."\n";
				echo '<td>'.$mfile['name']."</td>\n";
				echo '<td class="elx5_tabhide">'.$mfile['size']."</td>\n";
				echo '<td class="elx5_center">';
				echo '<a href="javascript:void(null);" class="elx5_btn elx5_ibtn" title="'.$eLang->get('ADD').'" onclick="addPluginCode(\'{video}'.$relpath.$mfile['name'].'{/video}\');"><i class="fas fa-location-arrow"></i></a>';
				echo '</td>'."\n";
				echo "</tr>\n";
			}
		} else {
			echo '<tr class="elx5_rowwarn"><td class="elx5_center" colspan="3">'.$eLang->get('NO_VIDEO_FILES')."</td></tr>\n";
		}
		echo "</tbody>\n";
		echo "</table>\n";
	}
	...
	private function uploadForm($pluginid, $fn) {
		...
		$form = new elxis5Form(array('idprefix' => 'plg'));
		$form->openForm(array('name' => 'plugvidform', 'method' =>'post', 'action' => $action, 'id' => 'plugvidform', 'enctype' => 'multipart/form-data'));
		$form->addNote($eLang->get('VIDEO_COMPOSER_INFO'), 'elx5_info');
...
?>
		<h3>File types and poster images</h3>
		<p>An HTML5 video can consist of several video files of different extensions and subtitles in 
		different languages. Poster images are also supported. The <strong>video files</strong> that belong to the same video should have the same name. 
		Example: <em>sample.ogv</em>, <em>sample.mp4</em>, <em>sample.webm</em>. <strong>Poster image</strong> (optional) should also have the same name 
		as the video and an jpg, jpeg or png extension. Example: <em>sample.jpg</em></p>

		<h3>Subtitles</h3>
		<p>You can optionally upload <strong>vtt or srt subtitles</strong>. The subtitles naming format is: {video_name}_{language}.vtt/srt<br />
		Examples: <em>sample_en.vtt</em>, <em>sample_el.vtt</em>, <em>sample_it.srt</em></p>

		<h3>Large files warning</h3>
		<p>Web servers have an <strong>upload limit</strong> (usually 2mb to 8mb), so if you try to upload larger files from here the action will fail. 
		...
		The Video plugin will automatically load all related files during playback. Don&apos;t forget to also set the plugin&apos;s general 
		configuration <strong>parameters</strong>.</p>

		<h3>HTML5 videos MIME support</h3>
		<p>If your server does not have <strong>MIME support</strong> for HTML5 videos then the videos wont play. In this case add in your site&apos;s htaccess file the following:</p>
<pre class="elx_textblock">AddType video/ogg .ogm

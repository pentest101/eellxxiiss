<?php 
/**
* @version		$Id: pagebreak.plugin.php 2136 2019-03-06 19:13:20Z IOS $
* @package		Elxis
...
class pagebreakPlugin implements contentPlugin {
	...
	public function process(&$row, $published, $params) {
		...
		if (preg_match('#^\<\/p\>#', $trimmed_text)) {
			$page_text = '<p class="elx5_zero">'.$page_text;
		} elseif (preg_match('#^\<\/div\>#', $trimmed_text)) {
			$page_text = '<div class="elx5_zero">'.$page_text;
		}
	...
	private function insertCode() {
		...
		echo '<div class="elx5_elx5_vsspace"><a href="javascript:void(null);" onclick="addPluginCode(\'{pagebreak}{/pagebreak}\');" class="elx5_btn"><i class="fas fa-location-arrow"></i> '.$eLang->get('CLICK_INSERT_PAGEBREAK')."</a></div>\n";
	}

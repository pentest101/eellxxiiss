...
function paramlang_switch(elname, isrtl) {
	...
	var eltype = 'input';
	if (textObj.tagName.toLowerCase() == 'textarea') { eltype = 'textarea'; }

	selObj.className = 'selectbox mlflag'+cur_lang;

	if (eltype == 'textarea') {
		textObj.className = 'text_area mlflag'+cur_lang;
	} else {
	...
	if (document.getElementById('params'+elname+'_ml'+cur_lang)) {
		...
		if (eltype == 'textarea') {
			var hidObj = document.createElement('textarea');
			hidObj.className = 'elx_invisible';	
		} else {

<?php 
/**
* @version		$Id: extension.xml.php 1495 2014-04-29 19:29:20Z sannosi $
* @package		Elxis
...
class extensionXML {
	...
	public function parse($xmlfile, $dependencies=false) {
		...
		if (isset($xmlDoc->tabs)) {
			$i = 1;
			$xmldir = dirname($xmlfile).'/';
			$eLang = eFactory::getLang();
			
			$onsave = '';
			$attrs = $xmlDoc->tabs->attributes();
			if ($attrs) {
				if (isset($attrs['onsave'])) { $onsave = (string)$attrs['onsave']; }
				if (trim($onsave) != '') {
					if (!file_exists($xmldir.$onsave) || !is_file($xmldir.$onsave)) {
						$onsave = '';
					} else {
						$onsave = $onsave;//relative path
					}
				}
			}

			foreach ($xmlDoc->tabs as $tabs) {
				...
				foreach ($tabs->children() as $tab) {
					...
					$contents = '';
					if ($attrs) {
						if (isset($attrs['label'])) { $label = (string)$attrs['label']; }
						if (isset($attrs['include'])) { $include = (string)$attrs['include']; $include = ltrim($include, '/'); }
					}
					...
					if (trim($include) != '') {
						if (!file_exists($xmldir.$include) || !is_file($xmldir.$include)) {
							$include = '';
						} else {
							$include = $xmldir.$include;//absolute path
						}
					} else {
						$contents = (string)$tab;
					}
					...
					if (($include != '') || ($contents != '')) {
						$this->tabs[] = array('label' => $label, 'include' => $include, 'contents' => $contents);
					}
				}
			}
			if ($this->tabs) { $this->tabsonsave = $onsave; }
		}
	...
	/***************************/
	/* GET TABS ON SAVE ACTION */
	/***************************/
	public function getTabsOnSave() {
		return $this->tabsonsave;
	}

<?php 
/**
* @version		$Id: installer.class.php 2311 2019-12-07 07:56:11Z IOS $
* @package		Elxis
...
class elxisInstaller {
	...
	private function check($aclcheck=true) {
		...
		if (file_exists($ipath) && is_dir($ipath)) {
			...
			if ($info['installed'] == false) {
				$this->errormsg = 'A folder found that should have the current version of the '.$this->head->type.' '.$this->head->name.' but the extension XML file was not found! If you previously uninstalled this extension but its folder remains delete that folder and re-try install.';
				$this->cleanUP();
				return false;
			}
	...
	private function notify($action, $type, $extension, $version) {
		...
		$body .= 'Powered by Elxis v'.$elxis->getVersion()." ( https://www.elxis.org )\r\n";
...
interface iExtension {
    public function install();
    public function uninstall($oldversion='');
    public function update();
}

<?php 
/**
* @version		$Id: mod_categories.php 2198 2019-04-09 17:42:08Z IOS $
* @package		Elxis
...
if (!class_exists('modCategories', false)) {
	class modCategories {
		...
		private function populate($categories) {
			...
			$addonclass = ($this->orientation == 1) ? ' elx5_modcateg_hor' : '';

			echo '<ul class="elx5_modcateg'.$sfx.$addonclass.'">'."\n";
			if ($this->home == 1) {
				$link = $eURI->makeURL();
				$home = $eLang->get('HOME');
				...
				foreach ($categories as $ctg) {
					...
					if (isset($ctg['subs']) && (count($ctg['subs']) > 0)) {
						foreach ($ctg['subs'] as $sctg) {
							...
							echo '<li class="elx5_modcateg_sub"><a href="'.$link.'" title="'.$sctg['title'].'">'.$sctg['title'].'</a></li>'."\n";
						}
					}
				}
			}

...
if (!class_exists('modOpenSearch', false)) {
	class modOpenSearch {
		...
		public function run() {
			...
			echo '<div class="elx5_opensearch elx5_center">'."\n";
			switch ($this->style) {
				case 1:
					echo '<a href="javascript:void(null);" onclick="installSearchEngine(\''.$osd.'\');" title="'.$eLang->get('ADD_ENGINE_BROWSER').'">';
					echo '<img src="'.$eURI->secureBase().'/components/com_search/extra/browsers.png" alt="open search" />'."</a>\n";
				break;
				case 2:
				...
				case 3:
					echo '<a href="javascript:void(null);" onclick="installSearchEngine(\''.$osd.'\');" title="'.$eLang->get('ADD_ENGINE_BROWSER').'">';
					echo '<img src="'.$eURI->secureBase().'/'.$this->custom_image.'" alt="open search" />'."</a>\n";
				break;
				case 0: default:
					...
					echo '<img src="'.$eURI->secureBase().'/components/com_search/extra/browsers.png" alt="open search" /><br />'."\n";

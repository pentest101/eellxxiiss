			...
			if ($this->mode == 1) {
				if (count($online['data']) > 0) {
					...
					foreach ($online['data'] as $user) {
						...
						if ($access == 2) {
							echo '<a href="'.$linkbase.$user['uid'].'.html" title="'.$title.'"><img src="'.$user['avatar'].'" alt="avatar" /></a>'."\n";
						} else {
							echo '<img src="'.$user['avatar'].'" alt="'.$user['name'].'" title="'.$title.'" />'."\n";
						}
					}
					echo "</div>\n";
				}
